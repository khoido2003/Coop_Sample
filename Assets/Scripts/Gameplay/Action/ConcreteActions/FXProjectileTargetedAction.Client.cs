...
namespace Unity.BossRoom.Gameplay.Actions
{
    public partial class FXProjectileTargetedAction
    {
        ...
        public override bool OnUpdateClient(ClientCharacter clientCharacter)
        {
            if (TimeRunning >= Config.ExecTimeSeconds && m_Projectile == null)
            {
                ...
                var initialDistance = Vector3.Distance(
                    targetPos,
                    clientCharacter.transform.position
                );
        ...
        void PlayHitReact()
        {
            ...
            if (
                m_Target
                && m_Target.TryGetComponent(out ServerCharacter clientCharacter)
                && clientCharacter.clientCharacter != null
            )
            {
                var hitReact = !string.IsNullOrEmpty(Config.ReactAnim)
                    ? Config.ReactAnim
                    : k_DefaultHitReact;
        ...
        NetworkObject GetTarget(ClientCharacter parent)
        {
            ...
            )
            {
                ...
                if (
                    targetable != null
                    && targetable.IsNpc == (Config.IsFriendly ^ parent.serverCharacter.IsNpc)
                )
                {
                    // not a valid target
                    return null;
            ...
            else
            {
                ...
                Debug.Log(
                    $"FXProjectileTargetedActionFX was targeted at ID {Data.TargetIds[0]}, but that target can't be found in spawned object list! (May have just been deleted?)"
                );
                return null;
            }
        }
        ...
        FXProjectile SpawnAndInitializeProjectile(ClientCharacter parent)
        {
            var projectileGO = Object.Instantiate(
                Config.Projectiles[0].ProjectilePrefab,
            ...
            if (!projectile)
            {
                throw new System.Exception(
                    $"FXProjectileTargetedAction tried to spawn projectile {projectileGO.name}, as dictated for action {name}, but the object doesn't have a FXProjectile component!"
                );
            }
        ...
        public override void AnticipateActionClient(ClientCharacter clientCharacter)
        {
            ...
            if (Data.TargetIds != null && Data.TargetIds.Length > 0)
            {
                var targetObj = NetworkManager.Singleton.SpawnManager.SpawnedObjects[
                    Data.TargetIds[0]
                ];
                if (targetObj)
                {
